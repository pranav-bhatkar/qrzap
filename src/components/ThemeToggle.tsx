...
export default function ThemeToggle() {
  ...
  const toggle = () => {
    ...
    window.posthog?.capture("theme_toggled", { theme: next ? "dark" : "light" });
  };

  return (
    <button
      onClick={toggle}
      className="rounded-md p-2 text-muted-foreground hover:text-foreground hover:bg-muted/30 transition-colors"
